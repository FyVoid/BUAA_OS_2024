...
int sys_clone(void *func, void *child_stack) {
	if (pa2page(PADDR(curenv->env_pgdir))->pp_ref >= 64) return -E_ACT_ENV_NUM_EXCEED;
	...
	e->env_tf.regs[29] = (u_int) child_stack;
...
int sys_mem_map(u_int srcid, u_int srcva, u_int dstid, u_int dstva, u_int perm) {
	...
	if ((pp = page_lookup(srcenv->env_pgdir, srcva, NULL)) == NULL) return -E_INVAL;
